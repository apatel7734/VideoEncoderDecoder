...
public class TextureMovieEncoder implements Runnable {
    ...
    public void frameAvailable(SurfaceTexture st, ArrayList<float[]> arrays) {
        synchronized (mReadyFence) {
            if (!mReady) {
                return;
            }
        ...
        arrays.add(transform);
        //Log.d(TAG, "Framenumber= " + mFrameNum);
        mHandler.sendMessage(mHandler.obtainMessage(MSG_FRAME_AVAILABLE, (int) (timestamp >> 32), (int) timestamp, arrays));
    }
    ...
    private void handleFrameAvailable(ArrayList<float[]> objs, long timestampNanos) {
        if (objs.size() > 0) {
            ...
            float[] second = objs.get(objs.size() - 2);
    ...
    private void drawBox2(int posn) {
        final int width = mInputWindowSurface.getWidth();
